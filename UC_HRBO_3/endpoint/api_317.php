...
include_once '../model/HRBO_317_Model.php';
...
if ($_POST["status"] == "A") {
    ...
    if ($_POST['message']) {
        $resp_noti = send_notification($host_notification_service, $_POST["emp_id"], "เบิกค่ารักษาพยาบาล", $_POST["message"]);

        if (json_decode($resp_noti)->code != "404") {
            $model_log_noti->empid       = $_POST["emp_id"];
            $model_log_noti->reqno       = $_POST["doc_no"];
            $model_log_noti->benefitcode = "medical_fee_approval";
    ...
    if ($_POST['money_back_amt']) {
        ...
        $model->money_back_date = $date_now;
