...
class HRBO_315_Model
{
    ...
    {
        $query = "SELECT * FROM myhr_req_rent_house_fee_approval As t WHERE 1=1 ";
    ...
    {
        $query = "SELECT * FROM myhr_req_rent_house_fee_approval AS t WHERE transaction_id=:id ";
         $stmt = $this->conn->prepare($query, array(PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL));
    ...
    {
        $query = "SELECT * FROM myhr_req_rent_house_fee_approval AS t WHERE t.emp_id=:empId AND status IN ('S','A','R')" ;
         $stmt = $this->conn->prepare($query, array(PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL));
    ...
    public function getallnoti()
    {
        ...
        $stmt->bindParam(":req_no", $this->rep);
        try {
            $stmt->execute();
            return $stmt;
        } catch (PDOException $ex) {
    ...
    {
        $query = "UPDATE myhr_req_rent_house_fee_approval SET status = :status WHERE transaction_id = :transaction_id";
    ...
    {
        $query = "UPDATE myhr_req_rent_house_fee_approval
