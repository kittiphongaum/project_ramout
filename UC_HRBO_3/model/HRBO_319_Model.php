...
class HRBO_319_Model
{
    ...
    public function search()
    {
        ...
        if (isset($this->membertype)) {      
            $query .= " AND t.member_type IN ('" . $this->membertype . "') ";
    ...
    {
        $query = "UPDATE myhr_req_beneficiary_item_db2 SET status_finish = :status WHERE doc_no = :doc_no";
    ...
    public function beneficiary_add(){

        $query = "SELECT json_finish from myhr_req_beneficiary_item WHERE doc_no =:doc_no";
        $stmt = $this->conn->prepare($query, array(PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL));
         $stmt->bindParam(":doc_no", $this->doc_no);
