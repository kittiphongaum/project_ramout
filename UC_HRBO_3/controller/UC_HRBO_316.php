...
$model->child_name      = isNotEmpty($_POST["child_name"]);
...
$rpt_url = $host_jasper_service . "/jasperserver/flow.html?j_username=" . $host_jasper_username . "&j_password=" . $host_jasper_password . "&_flowId=viewReportFlow&reportUnit=/reports/aomsinbo/UC_HRBO_316&decorate=no";
$rpt_url .= "&output=pdf";
...
$template->assign_var("child_name", $_POST["child_name"]);
$num = $stmt->rowCount();
if ($num > 0) {
    $response_data = array();
    $count         = 1;
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $newDate             = date("Y-m-d H:i", strtotime($row["req_date"]));
        $number = floor(($row["wdw_v7_amount"]*100))/100;
        $row["wdw_amount_String"] = number_format($number, 2);
