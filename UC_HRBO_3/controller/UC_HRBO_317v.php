...
require_once '../../UC_HRBO_3/model/HRBO_317_Model.php';
include '../../utils/base_function.php';

$template = new template();
$template->set_filenames(array(
    'body' => '../view/UC_HRBO_317v.html')
);

//get database connection
$database = new Database();
$db       = $database->getConnection();
$model    = new HRBO_317_Model($db);
...
$data_by_id["claim_treat_amount"] = number_format(floor(($data_by_id["claim_treat_amount"]*100))/100, 2);
$data_by_id["claim_amount"] = number_format(floor(($data_by_id["claim_amount"]*100))/100, 2);  
$data_by_id["claim_room_amount"] = number_format(floor(($data_by_id["claim_room_amount"]*100))/100, 2);
$data_by_id["claim_artificial_amount"] = number_format(floor(($data_by_id["claim_artificial_amount"]*100))/100, 2);
$data_by_id["claim_doctor_amount"] = number_format(floor(($data_by_id["claim_doctor_amount"]*100))/100, 2);
if ($data_by_id["req_additional_flag"] == 'Y') {
    $data_by_id["Req_additional_flag"] = "ใช่";
} else {
    $data_by_id["Req_additional_flag"] = "ไม่ใช่";
}
...
$data_by_id["req_date_"] =$data_by_id["req_date"];
