...
require_once '../../UC_HRBO_3/model/HRBO_329_Model.php';
include '../../utils/base_function.php';

$template = new template();
$template->set_filenames(array(
		'body' => '../view/UC_HRBO_329v.html')
);
//get database connection
$database = new Database();
$db       = $database->getConnection();
$model    = new HRBO_329_Model($db);
...
if ($data_by_id["status"] == "I") {
	$data_by_id["Status_"] = "<button id=\"approve\" type=\"button\" data-toggle=\"modal\" data-target=\"#md-approve\" class=\"btn btn-space btn-primary\">
...
// $model->EmpId =isNotEmpty($data_by_id["emp_id"]);
// $model->doc_no =$data_by_id["emp_id"];
// $memberty= $model->memberty();
// $num_noti = $memberty->rowCount();
//         if ($num_noti > 0) {
//             $count = 1;

//             while ($row = $memberty->fetch(PDO::FETCH_ASSOC)) {

//              $row["no"]  = $count;
//              $newDate = date("Y-m-d H:i", strtotime($row["created_date"]));
//              $row["created_date"]=$newDate;
//              $template->assign_block_vars('request3',$row);
//              var_dump($row);exit;
//              $count++;
//          }
// }
