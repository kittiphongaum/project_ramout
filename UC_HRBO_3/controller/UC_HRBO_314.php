...
$model    = new HRBO_314_Model($db);
    ...
    $rpt_url = $host_jasper_service . "/jasperserver/flow.html?j_username=" . $host_jasper_username . "&j_password=" . $host_jasper_password . "&_flowId=viewReportFlow&reportUnit=/reports/aomsinbo/UC_HRBO_314&decorate=no";
    $rpt_url .= "&output=pdf";
    $rpt_url .= "&req_no=" . isNotEmpty($_POST["req_no"]);
...
$template->assign_var("req_no",$_POST["req_no"]);
...
if ($num > 0) {
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $row["no"]           = $count;
        $row["priviledge_month"] = number_format(floor(($row["priviledge_month"]*100))/100, 2);
